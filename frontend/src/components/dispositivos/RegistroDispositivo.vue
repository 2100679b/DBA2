...
import sessionStorage from '@/services/SessionStore'
    ...
    async guardar() {
      try {
        const response = await axios.post(
          `${import.meta.env.VITE_API_URL}/api/dispositivos`,
          this.dispositivo
        )
        
        // (Opcional) Si tu backend responde con el nuevo dispositivo guardado
        const nuevoDispositivo = response.data

        // Agrega el dispositivo al store (si quieres mantener en frontend también)
        ...
        this.alerta.mensaje = 'No se pudo guardar el dispositivo. Intente más tarde.'
      }
    },
    limpiar() {
      this.dispositivo = {
